...
- (void)fakeSearch:(NSString*)text {
  ...
  if (text.length) {
    ...
    for (NSString* name in _allNames) {
		...
		// Checks for a substring match using predicates
		BOOL match = NO;  
		NSPredicate *containPred = [NSPredicate predicateWithFormat:@"SELF contains[cd] %@", text];  
		match = match | [containPred evaluateWithObject:name];  
		NSPredicate *matchPred = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", text];  
		match = match | [matchPred evaluateWithObject:name];  
